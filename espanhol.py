...
moderators = ["iMooseMoo","EL_LOCO_BUU","0AK4_x","__balmori__"]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")  
        await self.highrise.walk_to(Position(14.5 , 0.0 , 10.5 , "FrontLeft"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala") 
        if user.username in moderators:
         await self.highrise.chat(f"🚫{user.username} Moderador de la sala\n[Entró a la sala] ")
         
        await self.highrise.send_whisper(user.id,f"{user.username} Bienvenido a comprar y vender 💰 | Si necesitas ayuda, utiliza !ajuda")
...
Item(type='clothing', amount=1, id='bag-n_registrationavatars2023furrytail', account_bound=False, active_palette=-1),
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"Regresar @{user.username}! 😭")
        await self.highrise.send_emote(random.choice(emote))